"""Shared Hermes runtime helpers for WORKAGENT modules."""
...
_WORKAGENT_MCP_WAIT_TIMEOUT_SECONDS = 0.75
...
def prepare_hermes_home() -> None:
    """Point WORKAGENT child operations at the active Hermes profile directory."""
    try:
...
def _parse_enabled_flag(value, default: bool = True) -> bool:
    """Parse bool-like config values used by WORKAGENT MCP settings."""
    if value is None:
        return default
...
def _parse_workagent_mcp_active() -> bool | None:
    """Return the WORKAGENT MCP env override: True / False / None (auto)."""
    raw = os.environ.get("WORKAGENT_MCP_ACTIVE")
...
def _merge_workagent_mcp_toolsets(
    configured_toolsets: list[str],
    cfg: dict[str, object],
) -> list[str]:
    """Merge configured toolsets with WORKAGENT MCP server toolsets."""
    normalized_toolsets = [toolset for toolset in configured_toolsets if toolset != "no_mcp"]

    mcp_active = _parse_workagent_mcp_active()
...
def start_workagent_mcp_bootstrap(*, logger: logging.Logger | None = None) -> None:
    """Start Hermes-style background MCP discovery for WORKAGENT agent entrypoints."""
    if _parse_workagent_mcp_active() is False:
        return
    ...
    start_background_mcp_discovery(
        ...
        thread_name="workagent-mcp-discovery",
    )


def wait_for_workagent_mcp_bootstrap(timeout: float = _WORKAGENT_MCP_WAIT_TIMEOUT_SECONDS) -> None:
    """Bounded wait for WORKAGENT MCP discovery before first agent construction."""
    if _parse_workagent_mcp_active() is False:
        return
...
def _resolve_profile_enabled_toolsets(
    ...
    *,
    ...
    if configured_toolsets is None:
        ...
        return _merge_workagent_mcp_toolsets(normalized_toolsets, cfg)
...
def default_agent_factory(
    ...
    *,
    ...
    """Create a profile-configured AIAgent for WORKAGENT modules."""
    if os.environ.get("WORKAGENT_A2A_TEST_MODE") == "echo":
        return EchoAgent()

    active_logger = log or logger
    start_workagent_mcp_bootstrap(logger=active_logger)
    wait_for_workagent_mcp_bootstrap()
    ...
    log_prefix = "A2A" if "a2a" in platform else "WORKAGENT"
    active_logger.info(
    ...
    try:
        ...
        active_logger.warning(
            "WORKAGENT SessionDB unavailable; continuing without session persistence: %s",
            exc,
        )
