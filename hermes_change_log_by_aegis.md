...
Feature: Master switch for the unaddressed-group-message admission stage.
Intent: `HERMES_JEV_AUTOREPLY` (default off) is checked before the autoreply sub-switches, not after them. Off, there is no admission stage at all: Jev is never called and every message takes the pre-Jev path, because a verdict no caller acts on costs a round-trip of latency and one API call per group message for nothing — the sub-switches are inert rather than partially honoured, so `CHANNEL_AUTOREPLY=true` alone cannot start spending on judgements. On, an unaddressed group message is answered ONLY on an explicit allow: a denial, a transport error, an undecided verdict, a category the sub-switches do not cover, a missing API key or an empty agent description all end in silence. The sub-switches select which messages are ELIGIBLE to be judged, so they only ever narrow the master switch. This closes the fail-OPEN hole that mattered under `require_mention: false`, where "the normal path" means answer everything — an expired key or an emptied description would silently turn the bot into an unfiltered responder. The three-way outcome is resolved once in `channel_admission_mode()` (judge / silence / normal) rather than leaving callers to combine the flags, because the ordering is easy to re-derive wrongly and a wrong derivation fails open. DMs, @-mentioned messages, bot senders and slash commands keep their existing carve-outs. Scope is the gateway only: the WORKAGENT A2A service keeps its own rule (anything short of an explicit "no" is answered) because it is request/response with a caller blocking on a reply, and because a flag described as governing group chat must not silently change an RPC contract.

Feature: Separate opt-in for judging messages inside a topic.
Intent: `HERMES_JEV_THREAD_AUTOREPLY` (default off) scopes channel admission so it does not apply inside an existing group topic, where the agent is usually already a participant and re-judging every follow-up would cut a conversation off mid-way. Off, a thread message is left to the normal path rather than suppressed — the mention gate decides it exactly as before, so the flag narrows where Jev applies instead of adding a second way to stay silent. A message is "in a thread" when it carries `thread_id` or `root_id`; the FIRST message of a topic has neither (the bot creates the topic from its own reply) and is therefore always judged. The sub-switch cannot open the gate by itself.

Feature: Being asked for by name as a fourth admission proposition.
Intent: The admission verdict is now `(in_scope OR delegatable OR addressed) AND wants_answer`. Someone who types the assistant's name into a room has addressed it as deliberately as an @-mention, so refusing on topic grounds reads as the bot ignoring them — `addressed` is therefore a DISJUNCT, admitting a message the scope question rejects (measured: an off-topic request naming the assistant scores in_scope 0.21 / addressed 0.90 and is now answered). It stays a fourth noul rather than a clause on the scope question for the third time running: folding two propositions into one compound question has collapsed the calibration every time it has been measured, and Jev answers all four in one forward pass so separating them costs no extra round trip. `wants_answer` remains the AND term, not a fourth disjunct, so being named in a passing remark (addressed 0.24 / wants_answer 0.05) does not start a conversation. The question reads the name out of `agent` rather than taking a name setting of its own, which keeps one source of truth but means a description that never names the assistant leaves this proposition near zero and the feature inert. It is asked unconditionally, unlike `delegatable`, because `agent` is always present whenever the gate is active at all; a response missing ANY asked proposition is UNDECIDED rather than defaulted, since a missing disjunct would read as a silent false and a missing `wants_answer` as a silent true. Admission cost rises to ~678/75 tokens from ~471/39.

Feature: Admission judged against the agent, plus what it can delegate.
Intent: Channel relevance was judged against a separate hand-written `business_scope`, duplicating the description the complexity question already needs. The state now carries `agent` (from the shared `agent_description`) and an optional `remote_agents` inventory, because a request this agent would hand to a specialist is just as much its business as one it answers itself. `business_scope` is still read when `agent_description` is empty, so an existing deployment does not lose its gate on upgrade. Delegation is asked as its OWN proposition and ORed into the verdict, NOT folded into the scope question with an "or": the compound form was measured and collapsed the separation from ~0.85 to ~0.15, pushing an off-topic request to 0.62 against a 0.7 threshold while dragging genuine in-scope requests down to 0.77. Jev answers all three in one forward pass, so the extra proposition costs no extra round trip. The tool inventory deliberately stays out of admission: it rates how much work a request is, which is the complexity question's job.

Feature: Complexity as a four-way choice with configurable band criteria.
Intent: The complexity decision was a `score` over low/medium/high, which forced every request onto the low→high line — a request none of the bands described was still routed by whichever band it was pushed onto. It is now a `choice` over low/medium/high/`other`, where each of the three bands carries a criterion the deployment supplies (`HERMES_JEV_CRITERIA_<BAND>` / `jev.criteria.<band>`, merging env-over-config per band exactly as the band models do) and `other` is the residual that keeps the agent's default model. The band definitions had to move out of the instructions and into the options: they are configurable now, and instructions that also defined them would contradict an override rather than be replaced by it — a test asserts no default criterion appears in the instruction text. `other` is deliberately NOT configurable, being defined by the other three, and is rejected explicitly in `resolve_tier_model` so a stray `jev.models.other` cannot turn the residual into a fourth routable band. `judge_complexity` returns the string `"other"` rather than `None` so `complexity_scope: session` can remember it: it routes to the same default model as an undecided turn, but it is a decision rather than a failure, and re-asking it every turn would buy nothing. A blank criterion falls back to the built-in default instead of being sent empty, because an option with no description measurably loses calibration (the same four options asked about "hi" answered 0.35 confident with bare names and 1.00 with descriptions) and the answer is then gated on that confidence. Known consequence, measured and accepted: once `agent` is in the state, `other` also absorbs work outside that agent's subject area — a unique-index request reads as `medium` 0.85 with no context and `other` 0.88 for a SOC agent — so a narrow agent description narrows what gets re-routed. Two instruction wordings that pushed those rows back into bands were tried and rejected: they landed at 0.37-0.50 confidence, under the gate, so the turn reached the default model anyway through an unconfident path instead of a confident, legible `other`. Removing the confidence floor is part of the same change and not a separate relaxation: `HERMES_JEV_MIN_CONFIDENCE` / `jev.min_confidence` (default 0.5) existed because a thin, flat `score` distribution was the only way the model could say "I cannot place this", and `other` is that signal explicitly now. What the floor still did was discard CORRECT bands whose mass had merely split with the residual — a report-layout request bands `medium` at 0.30, right but not emphatic — routing them to the default model for a reason unrelated to the band being wrong; keeping both mechanisms also meant every criterion written moved confidence and so required re-tuning the threshold. The confidence stays on every log line as a diagnostic: measured, a request sitting near ~0.5 changes BAND between runs (`medium` 0.83 in one session, `low` 0.47-0.53 across five consecutive calls in another) while settled cases repeat at 0.98-0.99, so the figure is what distinguishes a criterion that separates the traffic from one that does not. With no floor, that instability now reaches the model choice, which is a further reason `complexity_scope` defaults to `session` — one decision per conversation rather than per turn.

Feature: Executor context in the complexity question.
Intent: Complexity is the cost of a task *as this agent would do it*, so the state carries an optional `agent` field alongside `request` — an under-specified alert batch moves from an unusable `high` 0.27 (discarded by `min_confidence`) to a usable `medium` 0.51 once the assistant is described as a SOC agent. `agent` is admitted where channel and sender are still refused, because it describes the executor and is stable per agent: it cannot make the same request band differently from one room to the next. The question is generated to name it only when it is actually sent, since referencing a state key the request does not carry is worse than sending no context. A `tools` inventory was carried here too and has been REMOVED along with `HERMES_JEV_TOOLS` and `jev.tools`: it was a second, hand-maintained copy of what the agent can do, it never tracked the live toolset (`platform_toolsets`, enabled plugins) so it mis-banded silently whenever it drifted, and the band criteria are configuration now, which is a strictly better lever for the same job — measured, a `low` criterion naming this deployment's lookups put a threat-intel query at `low` 0.95 where a four-tool inventory reached only `low` 0.77. Removing it also cut the `other` residual's over-reach: with the inventory in the state, ordinary `medium` work outside the agent's subject area (a unique index, a report layout) was answered `other` at 0.88-0.96; with `agent` alone both stay `medium`. The complexity state is now the closed set {request, agent}, asserted as such by a test, so a future field has to argue for itself.
