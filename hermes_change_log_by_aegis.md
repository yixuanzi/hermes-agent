...
Intent: Complexity is the cost of a task *as this agent would do it*, so the state carries optional `agent` and `tools` fields alongside `request` — a request one of the agent's tools answers directly is a `low` where the same request without it is a `medium` (measured: a threat-intel lookup moves from `medium` 0.54 to `low` 0.77, i.e. the band changes AND confidence rises). These are admitted where channel and sender are still refused, because they describe the executor and are stable per agent: they cannot make the same request band differently from one room to the next. The question is generated to name only the fields actually sent, since referencing a state key the request does not carry is worse than sending no context. The inventory accepts a plain string (env), a map, or a list, and is re-rendered at the point of use so the wire payload is always a readable block rather than a nested object. It is hand-maintained configuration, not the live toolset — deliberately, so that judgement inputs stay reviewable, at the cost of drifting if not maintained.
