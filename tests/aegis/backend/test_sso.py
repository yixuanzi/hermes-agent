...
def test_existing_users_database_is_migrated_without_recreating_users(tmp_path):
    database_path = Path(tmp_path) / "aegis.db"
...
def oidc_client(monkeypatch, load_backend, hermes_home):
    ...
    monkeypatch.setenv("OIDC_BACKCHANNEL_URL", "http://127.0.0.1:8080")
    monkeypatch.setenv("OIDC_CLIENT_ID", "aegis-test-client")
    monkeypatch.setenv("OIDC_CLIENT_SECRET", "test-secret")
    monkeypatch.setenv("OIDC_REDIRECT_URI", "http://127.0.0.1:9130/api/sso/callback")
...
def test_sso_start_uses_client_id_and_sso_without_subscription(oidc_client):
    ...
    assert query["client_id"] == ["aegis-test-client"]
    assert query["sso"] == ["1"]
...
def test_direct_start_rejects_wrong_client_id_and_preserves_only_org_context(oidc_client):
    ...
    response = client.get(
        "/api/sso/start?organization_id=org-1&client_id=aegis-test-client&subscription_id=secret",
        follow_redirects=False,
    )
    assert response.status_code == 302
...
def test_callback_validates_oidc_and_exchanges_ticket_once(oidc_client, monkeypatch):
    ...
    start = client.get(
        "/api/sso/start?organization_id=org-1&client_id=aegis-test-client",
        follow_redirects=False,
    )
    ...
    token, jwks = _signed_token("http://127.0.0.1:8080", "aegis-test-client", "member-1", "org-1", transaction["nonce"])
    module = __import__("aegis.backend.services.oidc_service", fromlist=["httpx"])
    monkeypatch.setattr(
        module.httpx,
        "AsyncClient",
...
def test_callback_rejects_direct_organization_mismatch(oidc_client, monkeypatch):
    ...
    token, jwks = _signed_token("http://127.0.0.1:8080", "aegis-test-client", "member-2", "org-2", transaction["nonce"])
    module = __import__("aegis.backend.services.oidc_service", fromlist=["httpx"])
    monkeypatch.setattr(
        module.httpx,
        "AsyncClient",
...
def test_existing_local_user_binds_by_email_but_admin_does_not(oidc_client, auth_headers):
    ...
    with pytest.raises(Exception) as exc_info:
        app.state.user_service.upsert_oidc_user(subject="member-admin", email="admin@aegis.local")
